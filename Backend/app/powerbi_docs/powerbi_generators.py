...
class PowerBIDocumentationGenerator:
    ...
    def apply_improvement_recommendation(self, context: Dict, recommendation: Dict, report_files: Dict, semantic_model_files: Dict) -> Dict:
        ...
        # System instruction specific to applying improvements
        apply_improvement_system_instruction = self.system_prompt + """
        
        TASK: Apply Improvement Recommendation
        You are a Power BI expert. Apply the specific improvement recommendation to the provided Power BI files.
        ...
        try:
            ...
            response = self.client.generate_content(
                prompt=final_user_prompt,
                system_instruction=apply_improvement_system_instruction,
                context='apply_improvement'
            )
            logger.info(f"Gemini response: {response}")
            try:
